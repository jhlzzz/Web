...
import food.mvc.model.FoodService;
...
public class FoodController extends HttpServlet {
	...
	private void list(HttpServletRequest request, HttpServletResponse response)
		    ...
		    ArrayList<Long> cpList = service.getCpList(ps, cp, maxPage);
		 	ArrayList<Food> list = service.listS(ps,cp);
		...
		private int getFnum(HttpServletRequest request){
	        int fnum = -1;
			String fnumStr = request.getParameter("fnum");
			if(fnumStr != null){
				fnumStr = fnumStr.trim();
				if(fnumStr.length() != 0){
					try{
		...
		private void delete(HttpServletRequest request, HttpServletResponse response) 
				throws ServletException, IOException {
			FoodService service = FoodService.getInstance();	
			int fnum = getFnum(request);
			service.deleteS(fnum);
		...
		private void content(HttpServletRequest request, HttpServletResponse response)
				...
				int fnum = getFnum(request);
				FoodService service = FoodService.getInstance();
				Food food = service.contentS(fnum);
